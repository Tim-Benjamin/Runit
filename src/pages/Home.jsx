...
export default function Home() {
  ...
  const filtered = activeFilter === "All" ? shops : shops.filter(s => s.category === activeFilter);

  return (
    <div style={{ background: 'var(--runit-bg)', minHeight: '100vh', color: 'var(--runit-text)' }}>

      {/* ── NAVBAR ── */}
      <nav className="pill-navbar-desktop" style={{
        position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100,
        background: 'rgba(10,31,28,0.85)', backdropFilter: 'blur(12px)',
        borderBottom: '0.5px solid var(--runit-border)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            ...
            fontWeight: 700, fontSize: 16, color: '#0a1f1c',
          }}>R</div>
          <span style={{ fontWeight: 700, fontSize: 18, color: 'var(--runit-text)' }}>RunIt</span>
        </div>

        {/* Desktop nav links */}
        <div style={{ display: 'flex', gap: 32, alignItems: 'center' }} className="desktop-nav">
          {['Home', 'Shops', 'How it works', 'Become a Runner'].map(link => (
            <a key={link} href={`#${link.toLowerCase().replace(/ /g, '-')}`}
              style={{ color: 'var(--runit-muted)', fontSize: 14, transition: 'color 0.2s' }}
              onMouseEnter={e => e.target.style.color = 'var(--runit-accent)'}
              onMouseLeave={e => e.target.style.color = 'var(--runit-muted)'}
            >{link}</a>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <Link to="/login" style={{
            ...
            border: '1px solid var(--runit-accent)',
            color: 'var(--runit-accent)', fontSize: 14, fontWeight: 500,
          }}>Login</Link>
          <Link to="/register" style={{
            padding: '8px 20px', borderRadius: 50,
            background: 'var(--runit-accent)',
            color: '#0a1f1c', fontSize: 14, fontWeight: 600,
          }}>Register</Link>
        </div>
      </nav>
      ...
      <section id="home" style={{
        ...
        padding: 'clamp(80px, 15vw, 120px) 24px 80px',
        background: 'radial-gradient(ellipse at 50% 40%, rgba(0,201,167,0.08) 0%, transparent 70%)',
        position: 'relative', overflow: 'hidden',
      }}>
        {/* Background circles */}
        <div style={{
          position: 'absolute', width: 600, height: 600, borderRadius: '50%',
          border: '1px solid rgba(0,201,167,0.06)', top: '50%', left: '50%',
          transform: 'translate(-50%,-50%)', pointerEvents: 'none',
        }} />
        <div style={{
          position: 'absolute', width: 400, height: 400, borderRadius: '50%',
          border: '1px solid rgba(0,201,167,0.08)', top: '50%', left: '50%',
          transform: 'translate(-50%,-50%)', pointerEvents: 'none',
        }} />
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, color: 'var(--runit-accent)', fontWeight: 500 }}>
            🎓 Built for UCC Students
          </span>
        </div>
        ...
        <h1 style={{
          ...
          background: 'linear-gradient(135deg, #e8f5f3 0%, #00c9a7 100%)',
        ...
        <p style={{
          fontSize: 18, color: 'var(--runit-muted)', maxWidth: 500,
          lineHeight: 1.6, marginBottom: 40,
        }}>
          Food, errands, shopping — describe what you need and a student runner delivers it to you. Fast. Cash on delivery.
        </p>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
          <Link to="/register" style={{
            ...
            boxShadow: '0 0 30px rgba(0,201,167,0.3)',
          }}>Place an Order</Link>
          <a href="#become-a-runner" style={{
            padding: '14px 32px', borderRadius: 50,
            border: '1px solid var(--runit-border-strong)',
            color: 'var(--runit-text)', fontWeight: 600, fontSize: 16,
          }}>Become a Runner</a>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 60, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[
            ...
            <div key={stat.label} style={{
              ...
              borderRadius: 50, padding: '10px 20px', textAlign: 'center',
            }}>
      ...
      <section id="how-it-works" style={{ padding: '80px 24px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
          {[
            ...
            { step: '02', icon: '🏃', title: 'A runner accepts', desc: 'A nearby student runner sees your order, accepts it, and heads out immediately.' },
            ...
            <div key={item.step} style={{
              ...
              <div style={{
                ...
                fontSize: 48, fontWeight: 800, color: 'rgba(0,201,167,0.06)', lineHeight: 1,
              }}>{item.step}</div>
      ...
      {/* ── CATEGORIES ── */}
      <section id="shops" style={{
        padding: '80px 24px',
        background: 'rgba(15,46,41,0.4)',
      }}>
      ...
      <section style={{ padding: '80px 24px', maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 12 }}>
          ...
          <span style={{
            background: 'rgba(0,201,167,0.1)', border: '1px solid var(--runit-border)',
            borderRadius: 50, padding: '4px 14px', fontSize: 12, color: 'var(--runit-muted)',
          }}>Marketplace coming soon</span>
        </div>
        <p style={{ color: 'var(--runit-muted)', marginBottom: 28, fontSize: 15 }}>
          Browse vendors — order through a runner, pay cash on delivery
        </p>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 28, overflowX: 'auto', paddingBottom: 4 }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveFilter(cat)} style={{
              ...
              borderColor: activeFilter === cat ? 'var(--runit-accent)' : 'var(--runit-border)',
              background: activeFilter === cat ? 'rgba(0,201,167,0.15)' : 'transparent',
              color: activeFilter === cat ? 'var(--runit-accent)' : 'var(--runit-muted)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {filtered.map(shop => (
            ...
            >
              <div style={{
                width: '100%', height: 100, background: 'var(--runit-elevated)',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                ...
                <span style={{
                  background: 'rgba(0,201,167,0.1)', borderRadius: 50,
                  padding: '3px 10px', fontSize: 11, color: 'var(--runit-accent)',
                  border: '1px solid var(--runit-border)', whiteSpace: 'nowrap',
                }}>{shop.category}</span>
              </div>
              <a href={`tel:${shop.phone}`} style={{
                fontSize: 12, color: 'var(--runit-muted)', display: 'block', marginBottom: 12,
              }}>📞 {shop.phone}</a>
              <button
                onClick={() => navigate('/register')}
                style={{
                  ...
                  background: 'rgba(0,201,167,0.1)', border: '1px solid var(--runit-border)',
                  color: 'var(--runit-accent)', fontWeight: 600, fontSize: 13,
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24 }}>
            {[
              ...
              { icon: '🎓', title: 'Student runners', desc: 'Every runner is a UCC student earning on their own schedule.' },
      ...
      {/* ── BECOME A RUNNER CTA ── */}
      <section id="become-a-runner" style={{
        padding: '100px 24px', textAlign: 'center',
        background: 'radial-gradient(ellipse at 50% 50%, rgba(0,201,167,0.07) 0%, transparent 70%)',
      }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            fontSize: 14, color: 'var(--runit-accent)',
          }}>
            Earn GH₵ 200–500+ per week
          </div>
          <br />
          <Link to="/register/runner" style={{
            display: 'inline-block', padding: '16px 40px', borderRadius: 50,
            background: 'var(--runit-accent)', color: '#0a1f1c',
            fontWeight: 700, fontSize: 16,
            boxShadow: '0 0 40px rgba(0,201,167,0.25)',
          }}>Sign up as a Runner</Link>
        </div>
      </section>

      {/* ── FOOTER ── */}
      <footer style={{
        background: 'var(--runit-surface)', borderTop: '1px solid var(--runit-border)',
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <div style={{
                  ...
                  background: 'var(--runit-accent)', display: 'flex',
                  alignItems: 'center', justifyContent: 'center',
                  fontWeight: 700, fontSize: 14, color: '#0a1f1c',
                }}>R</div>
                <span style={{ fontWeight: 700, fontSize: 16 }}>RunIt</span>
              </div>
              <p style={{ color: 'var(--runit-muted)', fontSize: 13, lineHeight: 1.6 }}>
                On-demand delivery for the University of Cape Coast community.
              </p>
            </div>
            <div>
              <div style={{ fontWeight: 600, marginBottom: 16, fontSize: 14 }}>Quick Links</div>
              {['Home', 'How it works', 'Become a Runner', 'Login', 'Register'].map(link => (
                <div key={link} style={{ marginBottom: 8 }}>
                  <a href="#" style={{ color: 'var(--runit-muted)', fontSize: 13 }}>{link}</a>
                </div>
              ))}
            </div>
            <div>
              <div style={{ fontWeight: 600, marginBottom: 16, fontSize: 14 }}>Contact</div>
              <p style={{ color: 'var(--runit-muted)', fontSize: 13, lineHeight: 1.8 }}>
          ...
          <div style={{
            borderTop: '1px solid var(--runit-border)', paddingTop: 24,
            display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12,
          }}>
            <span style={{ color: 'var(--runit-muted)', fontSize: 12 }}>© 2025 RunIt. All rights reserved.</span>
      ...
      <div className="mobile-topbar" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            ...
            background: 'var(--runit-accent)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontWeight: 800, fontSize: 15, color: '#0a1f1c',
          }}>R</div>
          <span style={{ fontWeight: 700, fontSize: 16, color: 'var(--runit-text)' }}>RunIt</span>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Link to="/login" style={{
            padding: '7px 16px', borderRadius: 50, fontSize: 13, fontWeight: 600,
            border: '1px solid var(--runit-accent)', color: 'var(--runit-accent)',
          }}>Login</Link>
          <Link to="/register" style={{
            padding: '7px 16px', borderRadius: 50, fontSize: 13, fontWeight: 600,
            background: 'var(--runit-accent)', color: '#0a1f1c',
